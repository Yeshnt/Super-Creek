...
// Ask the AI if this message warrants Creek joining in
...
client.on(Events.MessageCreate, async (message) => {
  ...
  if (content.length > 3 && Math.random() < 0.30) {
    try {
      ...
      // silently fail — emoji might not be valid
    }
  }

  // ── Randomly chime in if context warrants it (low chance) ─────────────────
  // 15% chance to even check, then AI decides
